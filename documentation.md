...
### 4.5 Stage 4: QA Validation (`qa_checks.py`)

Before export, deterministic (non-LLM) quality checks run:

| Check | Severity | What It Catches |
|-------|----------|----------------|
| `check_no_placeholders` | ERROR | `[EPS]`, `[value]`, `[TBD]` patterns |
| `check_bid_id_present` | WARNING | Bid number missing from executive summary |
| `check_buyer_present` | WARNING | Buyer org not mentioned |
| `check_no_hallucinated_numbers` | WARNING | Suspicious ungrounded numbers like "99%" accuracy |
| `check_section_not_empty` | ERROR | Blank or too-short sections |

### 4.6 Stage 5: Document Export
